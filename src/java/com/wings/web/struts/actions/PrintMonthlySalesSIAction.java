...
import com.wings.persistence.JobsheetDetailSI;
...
public final class PrintMonthlySalesSIAction extends Action {       
   ...
   private List getHandlingReportDetailSummary(HttpServletRequest request) throws IllegalAccessException, InvocationTargetException, Exception {
        ...
        globalList = com.wings.adapter.StrutsJobsheetDelegate.selectReportMonthlySalesSI(ds);  
        List iList = com.wings.adapter.StrutsJobsheetDelegate.selectReportMonthlySalesInvoiceSI(ds);
        //List debitList = com.wings.adapter.StrutsJobsheetDelegate.selectReportMonthlySalesDebit(ds);
        List finalList = new ArrayList();
        try {
            Query q = new Query ();            
            for (int i=0; i<globalList.size(); i++) {
                try {
                    JobsheetDetailSI jobDetail = (JobsheetDetailSI)globalList.get(i);
                    ...
                    for (int j=0;j<qrList.size();j++) {
                        try {
                            ...
                            if (j==0) {
                                ...
                                ms.setIncomingIDR(new Double(nList.get(1).toString()));
                                ms.setIncomingUSD(new Double(nList.get(2).toString()));
                                System.out.println("debug:"+jobDetail.getTotalExpensesIDR().doubleValue());
                                ms.setOutgoingIDR(new Double(jobDetail.getTotalExpensesIDR().doubleValue()));
                                ms.setOutgoingUSD(jobDetail.getTotalExpensesUSD());
                                ms.setOutgoingRefund(jobDetail.getRefund());
                                ms.setOutgoingRefundUS(jobDetail.getRefundUS());
