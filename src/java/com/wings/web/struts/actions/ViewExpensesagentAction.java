...
/**
 * Displays a expensesagent. We'll first try to look for a expensesagent
 ...
 */
public final class ViewExpensesagentAction extends Action {
   ...
   public ActionForward execute(
      ...
      ExpensesagentForm expensesagentForm = (ExpensesagentForm)request.getAttribute("expensesagent");
      if( expensesagentForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            expensesagentForm = new ExpensesagentForm();
            // Now set the form members with request values 
            RequestUtils.populate(expensesagentForm, request);

            expensesagentForm = com.wings.adapter.StrutsExpensesagentDelegate.edit(expensesagentForm);
      ...
      // Make a map that can be used to identify this Expensesagent in any links
      HashMap urlParams = new HashMap();
  urlParams.put("idExpensesAgent",expensesagentForm.getIdExpensesAgent());
