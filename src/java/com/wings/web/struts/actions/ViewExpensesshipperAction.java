...
/**
 * Displays a expensesshipper. We'll first try to look for a expensesshipper
 ...
 *
 * @struts.action
 *    path="/struts/viewExpensesshipper"
 *
 ...
 */
public final class ViewExpensesshipperAction extends Action {
   ...
   public ActionForward execute(
      ...
      ExpensesshipperForm expensesshipperForm = (ExpensesshipperForm)request.getAttribute("expensesshipper");
      if( expensesshipperForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            expensesshipperForm = new ExpensesshipperForm();
            // Now set the form members with request values 
            RequestUtils.populate(expensesshipperForm, request);

            expensesshipperForm = com.wings.adapter.StrutsExpensesshipperDelegate.edit(expensesshipperForm);
      ...
      // Make a map that can be used to identify this Expensesshipper in any links
      HashMap urlParams = new HashMap();
  urlParams.put("idExpensesShipper",expensesshipperForm.getIdExpensesShipper());
