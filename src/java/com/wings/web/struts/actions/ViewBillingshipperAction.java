...
/**
 * Displays a billingshipper. We'll first try to look for a billingshipper
 ...
 *
 * @struts.action
 *    path="/struts/viewBillingshipper"
 *
 ...
 */
public final class ViewBillingshipperAction extends Action {
   ...
   public ActionForward execute(
      ...
      BillingshipperForm billingshipperForm = (BillingshipperForm)request.getAttribute("billingshipper");
      if( billingshipperForm == null ) {
         // The vo wasn't on the request. We came via a direct link.
         // Get pk data from request object and use delegate to lookup
         try {
            billingshipperForm = new BillingshipperForm();
      ...
      // Make a map that can be used to identify this Billingshipper in any links
      HashMap urlParams = new HashMap();
  urlParams.put("idbillingShipper",billingshipperForm.getIdbillingShipper());
