/*
 * AgentServletAutocomplete.java
 *
 * Created on July 6, 2008, 3:02 PM
 */
 ...
 * @version
 */
public class AgentServletAutocomplete extends BaseAjaxServlet  {
    ...
    public String getXmlContent(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws Exception {
        try {
            String name = httpServletRequest.getParameter("agentName");
            ...
            /*
            List list = com.wings.adapter.StrutsAgentDelegate.findByName("%"+name+"%");

            // Create xml schema
            return new AjaxXmlBuilder().addItems(list, "name", "idAgent",true).toString();
            * 
            */
        } catch (Exception e) {
            return null;
